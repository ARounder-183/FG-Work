...
export async function GET() {
  const topics = await prisma.studyTopic.findMany({ orderBy: { createdAt: "asc" } });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await prisma.studyTopic.findUnique({ where: { name: name.trim() } });
    if (existing) return Response.json({ error: "主题已存在" }, { status: 409 });

    const topic = await prisma.studyTopic.create({
      data: { name: name.trim(), icon: icon || "📌" },
    });
